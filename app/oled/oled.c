...
{      			    
    ...
    {   
        ...
        for(t1=0;t1<8;t1++)
		{
			if(temp&0x80)OLED_DrawPoint(x,y,mode);//对应了字模软件的高位前，低位后
...
{ 	 		 
  GPIO_InitTypeDef  GPIO_InitStructure;
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA|RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOC|RCC_AHB1Periph_GPIOD|RCC_AHB1Periph_GPIOE|RCC_AHB1Periph_GPIOG, ENABLE);//使锟斤拷PORTA~E,PORTG时锟斤拷
  

#if OLED_MODE==1		//使用4线SPI 串口模式
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure);//锟斤拷始锟斤拷
	
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_7 ;	
	GPIO_Init(GPIOB, &GPIO_InitStructure);//锟斤拷始锟斤拷

  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_7|GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_11;	
	GPIO_Init(GPIOC, &GPIO_InitStructure);//锟斤拷始锟斤拷	
